            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_crop:
                        ...
                        #crop이미지의 가장 비율이 높은 색 출력
                        save_crop_path = str(save_dir / 'crops' / names[c] / f'{p.stem}.jpg' )
                        ct = ColorThief(save_crop_path)
                        dominant_color = ct.get_color(quality=1)
                        try:
                            cname = webcolors.rgb_to_name(dominant_color)
                            LOGGER.info(f"The color is exactly {cname}")
                        except ValueError:
                            cname = closest_color(dominant_color)
                            LOGGER.info(f"The color is closest to {cname}")
                        #TTS
                        audio = pyttsx3.init()
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                if len(det):
                    ...
                    for c in det[:, -1].unique():
                        ...
                        s += f"{n} {names[int(c)]}{'s' * (n > 1)}, "  # add to string
                        # UI에 label 출력
                        self.text.setText(str(names[int(c)]))
                ...
                cv2.cvtColor(im0, cv2.COLOR_BGR2RGB, im0) #UI 화면에 출력
                # UI 화면에 출력
